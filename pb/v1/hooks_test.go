...
func TestHookBitmapBitsMatchDescriptor(t *testing.T) {
	...
	ed := v1.Hook(0).Descriptor()
	if ed == nil {
		t.Fatal("Hook has no enum descriptor")
	}
	var seenNonZero int
	var wantMask v1.HookBitmap
	...
	if v1.MaxHookBit != 31 {
		t.Fatalf("MaxHookBit = %d, want 31 (u32 has 31 usable bits after reserved 0)", v1.MaxHookBit)
	}
	if got := v1.KnownHooksMask(); got != wantMask {
	...
	v1.Hook_name[30] = "HOOK_FAKE"
	t.Cleanup(func() { delete(v1.Hook_name, 30) })
	if v1.Hook(30).Bit() != 0 {
		t.Fatal("Hook.Bit must not honour a mutated Hook_name entry")
	}
	if v1.KnownHooksMask().Has(v1.Hook(30)) {
		t.Fatal("KnownHooksMask must not honour a mutated Hook_name entry")
	}
}

func TestValidateManifestHooksExactAgreement(t *testing.T) {
	before := v1.Hook_HOOK_BEFORE_REQUEST
	after := v1.Hook_HOOK_AFTER_RESPONSE

	t.Run("exact agreement", func(t *testing.T) {
		declared := []v1.Hook{before, after}
		if err := v1.ValidateManifestHooks(v1.BitmapOf(declared...), declared); err != nil {
			t.Fatalf("exact agreement must pass: %v", err)
		}
	})

	t.Run("empty declaration", func(t *testing.T) {
		err := v1.ValidateManifestHooks(0, nil)
	...
	t.Run("zero bitmap with declared hooks", func(t *testing.T) {
		err := v1.ValidateManifestHooks(0, []v1.Hook{before})
	...
	t.Run("missing hook", func(t *testing.T) {
		err := v1.ValidateManifestHooks(v1.BitmapOf(before), []v1.Hook{before, after})
		if err == nil || !strings.Contains(err.Error(), "missing") {
	...
	t.Run("extra registered hook", func(t *testing.T) {
		err := v1.ValidateManifestHooks(v1.BitmapOf(before, after), []v1.Hook{before})
	...
	t.Run("unknown declared enum", func(t *testing.T) {
		...
		err := v1.ValidateManifestHooks(0, []v1.Hook{v1.Hook(30)})
	...
	t.Run("HOOK_UNSPECIFIED declared", func(t *testing.T) {
		err := v1.ValidateManifestHooks(0, []v1.Hook{v1.Hook_HOOK_UNSPECIFIED})
	...
	t.Run("duplicate declared", func(t *testing.T) {
		err := v1.ValidateManifestHooks(v1.BitmapOf(before), []v1.Hook{before, before})
	...
	t.Run("reserved bit 0", func(t *testing.T) {
		err := v1.ValidateManifestHooks(v1.HookBitmap(1)|v1.BitmapOf(before), []v1.Hook{before})
	...
	t.Run("unknown high bit", func(t *testing.T) {
		...
		high := v1.HookBitmap(1) << 30
		err := v1.ValidateManifestHooks(high|v1.BitmapOf(before), []v1.Hook{before})
		if err == nil || !strings.Contains(err.Error(), "unexpected bits") {
	...
	t.Run("out of capacity declared", func(t *testing.T) {
		err := v1.ValidateManifestHooks(0, []v1.Hook{v1.Hook(32)})
...
func TestTickRequestIdIsNotRequestScoped(t *testing.T) {
	...
	if v1.Hook_HOOK_ON_TICK.RequestScoped() {
	...
	if v1.Hook_HOOK_UNSPECIFIED.RequestScoped() {
