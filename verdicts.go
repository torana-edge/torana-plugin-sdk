...
// Verdicts are attributed host calls. They are fire-and-forget: with
// all-or-nothing approval a refusal means the plugin asked for something it
// never declared, which lint catches at build time. The host logs the refusal.
//
...
func BlockRequest(status int32, code, message string) {
	_, _, _ = HostCall("env.block_request", &pbv2.BlockRequestArgs{
		Status:  status,
		Code:    code,
		Message: message,
	})
...
func RespondRequest(content string) {
	_, _, _ = HostCall("env.respond_request", &pbv2.RespondRequestArgs{Content: content})
}
...
func RouteRequest(provider, model string) {
	_, _, _ = HostCall("env.route_request", &pbv2.RouteRequestArgs{
		Provider: provider,
		Model:    model,
	})
}
...
func SetIdentity(identity string) {
	_, _, _ = HostCall("env.set_identity", &pbv2.SetIdentityArgs{Identity: identity})
}
