...
func TestChatResponseNarrowedPolicyContract(t *testing.T) {
	...
	args, _ := OutboundFieldPolicy("torana.v1.ToolCall", "arguments_json")
	...
	finish, _ := OutboundFieldPolicy("torana.v1.ChatResponse", "finish_reason")
	if !finish.IsHostOwned() {
		t.Fatal("ChatResponse.finish_reason must be host-owned")
	}
	id, _ := OutboundFieldPolicy("torana.v1.ToolCall", "id")
	if !id.IsHostOwned() {
	...
	for _, field := range []string{
		...
		"trailing_signature", "content_signature",
	} {
		if _, ok := OutboundFieldPolicy("torana.v1.ResponseMessage", field); ok {
			t.Errorf("ResponseMessage must not expose dead field %q", field)
		}
		if _, ok := OutboundFieldPolicy("torana.v1.Message", field); ok {
			t.Errorf("request Message must not remain in outbound inventory (found %q)", field)
		}
	}

	toolSig, _ := OutboundFieldPolicy("torana.v1.ToolCall", "signature")
